...
from ultralytics.nn.extra_modules.kan_convs.fast_kan_conv import FastKANConv2DLayer
...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl)  # strides computed during build
        self.stem = nn.ModuleList(nn.Sequential(GSConv(x, x, 3), GSConv(x, x, 1)) for x in ch)  # two 3x3 RepConv
